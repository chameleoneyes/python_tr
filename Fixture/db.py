...
class DBfixture:
    ...
    def get_contact_list(self):
        ...
        cursor = self.connection.cursor()
        try:
            cursor.execute("select id, firstname, lastname, mobile, email from addressbook "
                           "where deprecated='0000-00-00 00:00:00'")
            for row in cursor:
                (id, firstname, lastname, mobile, email) = row
                list.append(Contact(id=str(id), firstname=firstname, lastname=lastname, mobile=mobile, email1=email))
        finally:
            cursor.close()
        return list
